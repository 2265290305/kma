...
static int do_set_app_profile(void __user *arg)
{
	...
	pr_info("set_app_profile: key=%s uid=%d allow_su=%d version=%u\n",
		cmd.profile.key, cmd.profile.current_uid, cmd.profile.allow_su,
		cmd.profile.version);

	ret = ksu_set_app_profile(&cmd.profile);
	pr_info("set_app_profile: ret=%d key=%s uid=%d allow_su=%d\n", ret,
		cmd.profile.key, cmd.profile.current_uid, cmd.profile.allow_su);
	if (!ret) {
