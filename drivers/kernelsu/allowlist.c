...
bool ksu_get_app_profile(struct app_profile *profile)
{
	...
	list_for_each_entry_rcu (p, &allow_list, list) {
		...
		if (uid_match && key_match) {
			/*
			 * Keep get/set semantics aligned: when callers provide both uid and
			 * package name, return the exact matching profile instead of the first
			 * profile that happens to share the same uid.
			 */
...
static void do_persistent_allow_list(struct callback_head *_cb)
{
	...
	/*
	 * When the manager persists a grant it has usually already escaped to
	 * root. Keep that task context if possible and only fall back to ksu_cred
	 * for non-root workers such as the init task-work path.
	 */
	if (current_uid().val != 0 && ksu_cred)
