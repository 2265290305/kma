...
#define PARADISE_GYRO_MASK_ALL \
	(PARADISE_GYRO_MASK_GYRO | PARADISE_GYRO_MASK_UNCAL)
...
struct paradise_module_range {
	uintptr_t start;
	uintptr_t end;
	unsigned long vm_flags;
};

struct paradise_get_module_ranges_cmd {
	pid_t pid;
	char name[256];
	int vm_flag;
	uint32_t capacity;
...
struct paradise_process_alive_cmd {
	pid_t pid;
	int alive;
};
...
#define PARADISE_IOCTL_GET_MODULE_RANGES \
	_IOWR('W', 18, struct paradise_get_module_ranges_cmd)
#define PARADISE_IOCTL_READ_MEMORY _IOWR('W', 9, struct paradise_memory_cmd)
#define PARADISE_IOCTL_WRITE_MEMORY _IOWR('W', 12, struct paradise_memory_cmd)
#define PARADISE_IOCTL_READ_MEMORY_FAST \
	_IOWR('W', 16, struct paradise_memory_fast_cmd)
#define PARADISE_IOCTL_WRITE_MEMORY_FAST \
	_IOWR('W', 17, struct paradise_memory_fast_cmd)
#define PARADISE_IOCTL_HIDE_PATH _IOWR('W', 13, struct paradise_hide_path_cmd)
#define PARADISE_IOCTL_HIDE_PROCESS \
	_IOWR('W', 14, struct paradise_hide_process_cmd)
#define PARADISE_IOCTL_IS_PROCESS_ALIVE \
	_IOWR('W', 15, struct paradise_process_alive_cmd)
#define SHANZI_IOCTL_HIDE_MODULE _IOWR('W', 15, struct shanzi_hide_module_cmd)
#define PARADISE_IOCTL_LIST_PROCESSES_DECODED \
	_IOWR('W', 19, struct paradise_list_processes_cmd)
#define PARADISE_IOCTL_LIST_PROCESSES \
	_IOWR('W', 20, struct paradise_list_processes_cmd)
#define PARADISE_IOCTL_GYRO_CONFIG \
	_IOWR('W', 21, struct paradise_gyro_config_cmd)
...
#define SHANZI_IOCTL_HWBP_GET_NUM_BRPS _IOR('W', 32, struct shanzi_hwbp_u64_cmd)
#define SHANZI_IOCTL_HWBP_GET_NUM_WRPS _IOR('W', 33, struct shanzi_hwbp_u64_cmd)
...
#define SHANZI_IOCTL_HWBP_SET_HOOK_PC _IOWR('W', 40, struct shanzi_hwbp_u64_cmd)

#define SHANZI_HWBP_MAX_HITS 128

#define SHANZI_HWDEBUG_MAX_SLOTS 16

#define SHANZI_HWBP_THREAD_LOG_MAX 512

#define SHANZI_RING_CAPACITY 256
#define SHANZI_MODULE_RANGE_MAX 128
#define SHANZI_PROCESS_BITMAP_BYTES 8192
...
struct shanzi_virtual_target {
	...
	static DEFINE_MUTEX(shanzi_path_hide_lock);
	static LIST_HEAD(shanzi_hidden_paths);
	static DEFINE_MUTEX(shanzi_module_hide_lock);
	static DEFINE_MUTEX(shanzi_ring_lock);
	static DEFINE_MUTEX(g_touch_lock);
	static DEFINE_MUTEX(shanzi_gyro_lock);
	...
	static int (*shanzi_kern_path_fn)(const char *name, unsigned int flags,
					  struct path *path);
	static void (*shanzi_path_put_fn)(const struct path *path);
	static int (*shanzi_path_umount_fn)(struct path *path, int flags);
	static int (*shanzi_path_mount_fn)(const char *dev_name,
					   struct path *path,
	...
	static struct kprobe shanzi_reboot_kp;
	static bool shanzi_gyro_enabled;
	static uint32_t shanzi_gyro_type_mask = PARADISE_GYRO_MASK_ALL;
	static float shanzi_gyro_x;
	static float shanzi_gyro_y;

	enum shanzi_supercall_abi {
		SHANZI_SUPERCALL_ABI_DIRECT,
	...
	static uint32_t shanzi_hwdebug_info_value(bool is_break)
	{
		...
		uint32_t slots = is_break ? shanzi_get_num_brps() :
					    shanzi_get_num_wrps();
	...
	static void shanzi_virtual_target_get(pid_t tgid)
	{
		...
		if (entry) {
			entry->refs++;
			spin_unlock_irqrestore(&shanzi_virtual_target_lock,
					       flags);
			return;
		}
						    ...
						    struct iovec __user *uiov)
	{
		...
		if (copy_len &&
		    copy_from_user_nofault(&local, iov.iov_base, copy_len))
			return -EFAULT;
		...
		if (copy_len > offsetof(struct user_hwdebug_state, dbg_regs))
			reg_bytes =
				copy_len -
				offsetof(struct user_hwdebug_state, dbg_regs);
		slots_to_copy = min_t(uint32_t,
				      reg_bytes / sizeof(local.dbg_regs[0]),
		...
		max_slots = is_break ? shanzi_get_num_brps() :
				       shanzi_get_num_wrps();
		...
		slot_count = shanzi_hwdebug_slot_count(
			regs, SHANZI_HWDEBUG_MAX_SLOTS);
		if (is_break) {
		...
		SHZ_INFO(
			"Shanzi: fake hwdebug set tgid=%d tid=%d kind=%s copy_len=%zu slots=%u max=%u ret=%ld first=[0x%llx/0x%x]\n",
						    ...
						    struct iovec __user *uiov)
	{
		...
		if (copy_len &&
		    copy_to_user_nofault(iov.iov_base, &local, copy_len))
			return -EFAULT;

		iov.iov_len = copy_len;
		SHZ_INFO(
			"Shanzi: fake hwdebug get tgid=%d tid=%d kind=%s copy_len=%zu info=0x%x first=[0x%llx/0x%x]\n",
			tgid, tid, is_break ? "break" : "watch", copy_len,
			local.dbg_info,
						  ...
						  unsigned long data)
	{
		...
		if (request == PTRACE_SETREGSET)
			return shanzi_emulate_ptrace_setregset(
				tgid, tid, note_type,
				(struct iovec __user *)data);
		if (request == PTRACE_GETREGSET)
			return shanzi_emulate_ptrace_getregset(
				tgid, tid, note_type,
				(struct iovec __user *)data);
					  ...
					  struct pt_regs *regs)
	{
		...
		ret = shanzi_emulate_ptrace_hwdebug(child, request, note_type,
						    data);
		if (ret == LONG_MIN)
			return 0;

		SHZ_INFO(
			"Shanzi: ptrace virtualized req=%ld tgid=%d tid=%d type=0x%lx ret=%ld\n",
	...
	static int shanzi_install_ptrace_virtualization(void)
	{
		...
		memset(&shanzi_arch_ptrace_kp, 0,
		       sizeof(shanzi_arch_ptrace_kp));
...
#define SHZ_DEC(name, key, ...)        \
	...
	{
		SHZ_DEC(kallsyms_name, 0x5A, 'k' ^ 0x5A, 'a' ^ 0x5A, 'l' ^ 0x5A,
			'l' ^ 0x5A, 's' ^ 0x5A, 'y' ^ 0x5A, 'm' ^ 0x5A,
			's' ^ 0x5A, '_' ^ 0x5A, 'l' ^ 0x5A, 'o' ^ 0x5A,
			'o' ^ 0x5A, 'k' ^ 0x5A, 'u' ^ 0x5A, 'p' ^ 0x5A,
			'_' ^ 0x5A, 'n' ^ 0x5A, 'a' ^ 0x5A, 'm' ^ 0x5A,
			'e' ^ 0x5A, 0);
		struct kprobe kp = {
	...
	{
		SHZ_DEC(sym_kern_path, 0x31, 'k' ^ 0x31, 'e' ^ 0x31, 'r' ^ 0x31,
			'n' ^ 0x31, '_' ^ 0x31, 'p' ^ 0x31, 'a' ^ 0x31,
			't' ^ 0x31, 'h' ^ 0x31, 0);
		SHZ_DEC(sym_path_put, 0x31, 'p' ^ 0x31, 'a' ^ 0x31, 't' ^ 0x31,
			'h' ^ 0x31, '_' ^ 0x31, 'p' ^ 0x31, 'u' ^ 0x31,
			't' ^ 0x31, 0);
		SHZ_DEC(sym_path_umount, 0x31, 'p' ^ 0x31, 'a' ^ 0x31,
			't' ^ 0x31, 'h' ^ 0x31, '_' ^ 0x31, 'u' ^ 0x31,
			'm' ^ 0x31, 'o' ^ 0x31, 'u' ^ 0x31, 'n' ^ 0x31,
			't' ^ 0x31, 0);
		SHZ_DEC(sym_path_mount, 0x31, 'p' ^ 0x31, 'a' ^ 0x31,
			't' ^ 0x31, 'h' ^ 0x31, '_' ^ 0x31, 'm' ^ 0x31,
			'o' ^ 0x31, 'u' ^ 0x31, 'n' ^ 0x31, 't' ^ 0x31, 0);

		if (!shanzi_kern_path_fn)
			shanzi_kern_path_fn =
				(void *)shanzi_lookup_symbol(sym_kern_path);
		if (!shanzi_path_put_fn)
			shanzi_path_put_fn =
				(void *)shanzi_lookup_symbol(sym_path_put);
		if (!shanzi_path_umount_fn)
			shanzi_path_umount_fn =
				(void *)shanzi_lookup_symbol(sym_path_umount);
		if (!shanzi_path_mount_fn)
			shanzi_path_mount_fn =
				(void *)shanzi_lookup_symbol(sym_path_mount);
		...
		if (!shanzi_init_task_ptr)
			shanzi_init_task_ptr =
				(void *)shanzi_lookup_symbol("init_task");
		return 0;
	}

	static int shanzi_resolve_module_hide_helpers(void)
	{
		SHZ_DEC(sym_module_mutex, 0x24, 'm' ^ 0x24, 'o' ^ 0x24,
			'd' ^ 0x24, 'u' ^ 0x24, 'l' ^ 0x24, 'e' ^ 0x24,
			'_' ^ 0x24, 'm' ^ 0x24, 'u' ^ 0x24, 't' ^ 0x24,
			'e' ^ 0x24, 'x' ^ 0x24, 0);
	...
	static void shanzi_remove_module_usage_links(struct module *mod)
	{
#ifdef CONFIG_MODULE_UNLOAD
		struct module_use *use, *tmp;

		list_for_each_entry_safe(use, tmp, &mod->target_list,
					 target_list) {
	...
	static long shanzi_hide_module_like_66(void)
	{
		long ret;
		...
		if (!shanzi_module_hidden) {
			...
			shanzi_remove_module_usage_links(THIS_MODULE);
			shanzi_module_hidden = true;
			SHZ_INFO("Shanzi: module hidden like 6.6\n");
		}
		ret = 0;
	...
	static long shanzi_ioctl_hide_module(unsigned long arg)
	{
		...
		if (!cmd.hide)
			return -EOPNOTSUPP;
		return shanzi_hide_module_like_66();
	}

	static void shanzi_hide_module_auto(void)
	{
		if (shanzi_hide_module_like_66() == 0)
	...
	static int shanzi_hide_mount_tmpfs(const char *path)
	{
		...
		SHZ_DEC(mount_data, 0x11, 's' ^ 0x11, 'i' ^ 0x11, 'z' ^ 0x11,
			'e' ^ 0x11, '=' ^ 0x11, '0' ^ 0x11, ',' ^ 0x11,
			'm' ^ 0x11, 'o' ^ 0x11, 'd' ^ 0x11, 'e' ^ 0x11,
			'=' ^ 0x11, '0' ^ 0x11, '5' ^ 0x11, '5' ^ 0x11,
			'5' ^ 0x11, 0);
		SHZ_DEC(tmpfs_name, 0x22, 't' ^ 0x22, 'm' ^ 0x22, 'p' ^ 0x22,
			'f' ^ 0x22, 's' ^ 0x22, 0);
		int ret;
		...
		ret = shanzi_path_mount_fn(tmpfs_name, &mount_path, tmpfs_name,
					   0, mount_data);
	...
	static void
	shanzi_cleanup_hidden_proc(struct shanzi_hidden_proc *hidden)
	{
		char path_buf[64];
		...
		if (hidden->tasks_unlinked && shanzi_tasklist_lock_ptr &&
			...
			list_add_tail(&hidden->task->tasks,
				      &shanzi_init_task_ptr->tasks);
		...
		if (hidden->proc_mounted) {
			snprintf(path_buf, sizeof(path_buf), "/proc/%u",
				 hidden->pid);
		...
		if (hidden->kgsl_mounted) {
			snprintf(path_buf, sizeof(path_buf),
				 "/sys/devices/virtual/kgsl/kgsl/proc/%u",
				 hidden->pid);
			shanzi_hide_umount_path(path_buf);
		}
	...
	static void shanzi_cleanup_all_hidden_procs(void)
	{
		...
		list_for_each_entry_safe(entry, tmp, &shanzi_hidden_procs,
					 link) {
			list_del(&entry->link);
	...
	static struct shanzi_hidden_path *
	shanzi_find_hidden_path(const char *path)
	{
		struct shanzi_hidden_path *entry;

		list_for_each_entry(entry, &shanzi_hidden_paths, link) {
			if (!strncmp(entry->path, path, sizeof(entry->path)))
				return entry;
		}
	...
	static void
	shanzi_cleanup_hidden_path(struct shanzi_hidden_path *hidden)
	{
		if (!hidden)
			return;

		if (hidden->mounted)
			shanzi_hide_umount_path(hidden->path);
		kfree(hidden);
	}

	static void shanzi_cleanup_all_hidden_paths(void)
	{
		struct shanzi_hidden_path *entry;
		struct shanzi_hidden_path *tmp;

		mutex_lock(&shanzi_path_hide_lock);
		list_for_each_entry_safe(entry, tmp, &shanzi_hidden_paths,
					 link) {
			list_del(&entry->link);
			shanzi_cleanup_hidden_path(entry);
		}
		mutex_unlock(&shanzi_path_hide_lock);
	}
	...
	{
		return ((shanzi_ring_head + 1) % SHANZI_RING_CAPACITY) ==
		       shanzi_ring_tail;
	}

	static __maybe_unused int shanzi_ring_queue_push(uint32_t type,
							 int32_t slot,
							 int32_t x, int32_t y)
	{
		...
		evt->y = y;
		shanzi_ring_head =
			(shanzi_ring_head + 1) % SHANZI_RING_CAPACITY;
		mutex_unlock(&shanzi_ring_lock);
	...
	{
		SHZ_DEC(sym_input_dev_list, 0x33, 'i' ^ 0x33, 'n' ^ 0x33,
			'p' ^ 0x33, 'u' ^ 0x33, 't' ^ 0x33, '_' ^ 0x33,
			'd' ^ 0x33, 'e' ^ 0x33, 'v' ^ 0x33, '_' ^ 0x33,
			'l' ^ 0x33, 'i' ^ 0x33, 's' ^ 0x33, 't' ^ 0x33, 0);
		SHZ_DEC(sym_input_mutex, 0x33, 'i' ^ 0x33, 'n' ^ 0x33,
			'p' ^ 0x33, 'u' ^ 0x33, 't' ^ 0x33, '_' ^ 0x33,
			'm' ^ 0x33, 'u' ^ 0x33, 't' ^ 0x33, 'e' ^ 0x33,
			'x' ^ 0x33, 0);
		SHZ_DEC(sym_input_event, 0x33, 'i' ^ 0x33, 'n' ^ 0x33,
			'p' ^ 0x33, 'u' ^ 0x33, 't' ^ 0x33, '_' ^ 0x33,
			'e' ^ 0x33, 'v' ^ 0x33, 'e' ^ 0x33, 'n' ^ 0x33,
			't' ^ 0x33, 0);

		if (!shanzi_input_dev_list_ptr)
			shanzi_input_dev_list_ptr =
				(struct list_head *)shanzi_lookup_symbol(
					sym_input_dev_list);
		if (!shanzi_input_mutex_ptr)
			shanzi_input_mutex_ptr =
				(struct mutex *)shanzi_lookup_symbol(
					sym_input_mutex);
		if (!shanzi_input_event_fn)
			shanzi_input_event_fn =
				(void *)shanzi_lookup_symbol(sym_input_event);

		if (!shanzi_input_dev_list_ptr || !shanzi_input_mutex_ptr ||
		    !shanzi_input_event_fn)
			return -ENOENT;
		return 0;
	}
	...
	static void shanzi_emit_touch_sync_locked(void)
	{
		...
		shanzi_input_event_fn(g_touch_dev, EV_KEY, BTN_TOUCH,
				      any_active ? 1 : 0);
		shanzi_input_event_fn(g_touch_dev, EV_KEY, BTN_TOOL_FINGER,
				      any_active ? 1 : 0);
		shanzi_input_event_fn(g_touch_dev, EV_SYN, SYN_REPORT, 0);
	}

	static long shanzi_touch_inject_event(uint32_t type, int32_t slot,
					      int32_t x, int32_t y)
	{
		...
		case 0:
			...
			touch->x = x;
			touch->y = y;
			shanzi_input_event_fn(g_touch_dev, EV_ABS,
					      ABS_MT_TRACKING_ID,
					      touch->tracking_id);
			shanzi_input_event_fn(g_touch_dev, EV_ABS,
					      ABS_MT_POSITION_X, x);
	...
	static long hello_ioctl_touch_down(unsigned long arg)
	{
		...
		if (shanzi_touch_mode == SHANZI_TOUCH_MODE_RING ||
			...
			return shanzi_ring_queue_push(0, cmd.slot, cmd.x,
						      cmd.y);
	...
	static long hello_ioctl_touch_move(unsigned long arg)
	{
		...
		if (shanzi_touch_mode == SHANZI_TOUCH_MODE_RING ||
		    shanzi_touch_mode == SHANZI_TOUCH_MODE_RING_HOOK)
			return shanzi_ring_queue_push(2, cmd.slot, cmd.x,
						      cmd.y);
	...
	static long hello_ioctl_touch_set_mode(unsigned long arg)
	{
		...
		if (shanzi_touch_mode == SHANZI_TOUCH_MODE_RING_HOOK)
			SHZ_INFO(
				"Shanzi: touch ring hook mode requested, using ring fallback for now\n");
		return 0;
	}

	static int shanzi_resolve_hwbp_helpers(void)
	{
		SHZ_DEC(sym_reg_hwbp, 0x4D, 'r' ^ 0x4D, 'e' ^ 0x4D, 'g' ^ 0x4D,
			'i' ^ 0x4D, 's' ^ 0x4D, 't' ^ 0x4D, 'e' ^ 0x4D,
			'r' ^ 0x4D, '_' ^ 0x4D, 'u' ^ 0x4D, 's' ^ 0x4D,
			'e' ^ 0x4D, 'r' ^ 0x4D, '_' ^ 0x4D, 'h' ^ 0x4D,
			'w' ^ 0x4D, '_' ^ 0x4D, 'b' ^ 0x4D, 'r' ^ 0x4D,
			'e' ^ 0x4D, 'a' ^ 0x4D, 'k' ^ 0x4D, 'p' ^ 0x4D,
			'o' ^ 0x4D, 'i' ^ 0x4D, 'n' ^ 0x4D, 't' ^ 0x4D, 0);
		SHZ_DEC(sym_mod_hwbp, 0x4D, 'm' ^ 0x4D, 'o' ^ 0x4D, 'd' ^ 0x4D,
			'i' ^ 0x4D, 'f' ^ 0x4D, 'y' ^ 0x4D, '_' ^ 0x4D,
			'u' ^ 0x4D, 's' ^ 0x4D, 'e' ^ 0x4D, 'r' ^ 0x4D,
			'_' ^ 0x4D, 'h' ^ 0x4D, 'w' ^ 0x4D, '_' ^ 0x4D,
			'b' ^ 0x4D, 'r' ^ 0x4D, 'e' ^ 0x4D, 'a' ^ 0x4D,
			'k' ^ 0x4D, 'p' ^ 0x4D, 'o' ^ 0x4D, 'i' ^ 0x4D,
			'n' ^ 0x4D, 't' ^ 0x4D, 0);
		SHZ_DEC(sym_unreg_hwbp, 0x4D, 'u' ^ 0x4D, 'n' ^ 0x4D,
			'r' ^ 0x4D, 'e' ^ 0x4D, 'g' ^ 0x4D, 'i' ^ 0x4D,
			's' ^ 0x4D, 't' ^ 0x4D, 'e' ^ 0x4D, 'r' ^ 0x4D,
			'_' ^ 0x4D, 'h' ^ 0x4D, 'w' ^ 0x4D, '_' ^ 0x4D,
			'b' ^ 0x4D, 'r' ^ 0x4D, 'e' ^ 0x4D, 'a' ^ 0x4D,
			'k' ^ 0x4D, 'p' ^ 0x4D, 'o' ^ 0x4D, 'i' ^ 0x4D,
			'n' ^ 0x4D, 't' ^ 0x4D, 0);
...
#define SHANZI_GEN_WRITE_WB_REG_CASES(OFF, REG, VAL) \
	...
	static uint64_t shanzi_read_wb_reg(int reg, int n)
	{
		...
		switch (reg + n) {
			SHANZI_GEN_READ_WB_REG_CASES(AARCH64_DBG_REG_BVR,
						     AARCH64_DBG_REG_NAME_BVR,
						     val);
			SHANZI_GEN_READ_WB_REG_CASES(AARCH64_DBG_REG_BCR,
						     AARCH64_DBG_REG_NAME_BCR,
						     val);
			SHANZI_GEN_READ_WB_REG_CASES(AARCH64_DBG_REG_WVR,
						     AARCH64_DBG_REG_NAME_WVR,
						     val);
			SHANZI_GEN_READ_WB_REG_CASES(AARCH64_DBG_REG_WCR,
	...
	{
		switch (reg + n) {
			SHANZI_GEN_WRITE_WB_REG_CASES(AARCH64_DBG_REG_BVR,
						      AARCH64_DBG_REG_NAME_BVR,
						      val);
			SHANZI_GEN_WRITE_WB_REG_CASES(AARCH64_DBG_REG_BCR,
						      AARCH64_DBG_REG_NAME_BCR,
						      val);
			SHANZI_GEN_WRITE_WB_REG_CASES(AARCH64_DBG_REG_WVR,
						      AARCH64_DBG_REG_NAME_WVR,
						      val);
			SHANZI_GEN_WRITE_WB_REG_CASES(AARCH64_DBG_REG_WCR,
						      AARCH64_DBG_REG_NAME_WCR,
						      val);
		default:
			break;
	...
	static bool
	shanzi_toggle_bp_registers_directly(const struct perf_event_attr *attr,
					    int enable)
	{
		int i;
		int max_slots;
	...
	static int read_process_cmdline(struct task_struct *task, char *buffer,
					size_t buflen)
	{
		struct mm_struct *mm;
		...
		copied = access_process_vm(task, arg_start, buffer,
					   arg_end - arg_start, FOLL_FORCE);
		if (copied > 0) {
			char *space;
			char *slash;
...
out_mm:
	...
	static int find_process_by_name(const char *name)
	{
		...
		needle_len = strnlen(
			name, sizeof(((struct paradise_get_pid_cmd *)0)->name));
		if (!needle_len)
			return -EINVAL;
		...
		for (i = 0; i < count; i++) {
			...
			task = tasks[i];
			if (read_process_cmdline(task, cmdline,
						 sizeof(cmdline)) > 0) {
				size_t cmd_len =
					strnlen(cmdline, sizeof(cmdline));

				if (cmd_len == needle_len &&
				    !strncmp(cmdline, name, needle_len)) {
			...
			if (comm_len == needle_len &&
			    !strncmp(comm, name, needle_len)) {
				pid = task_pid_nr(task);
				put_task_struct(task);
				break;
			}
	...
	static bool get_module_bounds(pid_t pid, const char *name,
				      ...
				      uintptr_t *base_out, uintptr_t *end_out)
	{
		...
		needle_len = strnlen(
			name, sizeof(((struct paradise_get_module_base_cmd *)0)
					     ->name));
		if (!needle_len)
			return false;
		...
		for_each_vma(vmi, vma)
		{
			...
			if (file) {
				...
				matched = !bcmp(dname, name, match_len);
			} else if (found && (uintptr_t)vma->vm_start == end &&
			...
			if (!matched)
				continue;
			if (vm_flag && (vma->vm_flags & vm_flag) != vm_flag)
				continue;
	...
	static bool module_vma_name_matches(struct vm_area_struct *vma,
					    const char *name, size_t needle_len)
	{
		struct file *file;
		const char *dname;
		size_t match_len;
		...
		return !bcmp(dname, name, match_len);
	}

	static bool module_vma_is_adjacent_bss(struct vm_area_struct *vma,
					       uintptr_t module_end)
	{
		if (vma->vm_file)
			return false;
		if ((uintptr_t)vma->vm_start != module_end)
			return false;
	...
	static long
	get_module_ranges(pid_t pid, const char *name, unsigned long vm_flag,
			  struct paradise_module_range __user *ranges,
			  uint32_t capacity, uint32_t *count_out)
	{
		...
		struct mm_struct *mm;
		struct vm_area_struct *vma;
		struct paradise_module_range *items = NULL;
		size_t needle_len;
		uint32_t total = 0;
		...
		*count_out = 0;
		if (capacity > SHANZI_MODULE_RANGE_MAX)
			return -EINVAL;

		needle_len = strnlen(
			name,
			sizeof(((struct paradise_get_module_ranges_cmd *)0)
				       ->name));
		if (!needle_len)
			return -EINVAL;
		if (capacity && !ranges)
			return -EINVAL;
		if (capacity) {
			items = kcalloc(capacity, sizeof(*items), GFP_KERNEL);
			if (!items)
				return -ENOMEM;
		}
		...
		for_each_vma(vmi, vma)
		{
			...
			matched =
				module_vma_name_matches(vma, name, needle_len);
			if (!matched && module_seen)
				matched = module_vma_is_adjacent_bss(
					vma, module_end);
			if (matched) {
				module_seen = true;
				if ((uintptr_t)vma->vm_end > module_end)
					module_end = (uintptr_t)vma->vm_end;
			}
			...
			if (total < capacity) {
				items[total].start = (uintptr_t)vma->vm_start;
				items[total].end = (uintptr_t)vma->vm_end;
				items[total].vm_flags = vma->vm_flags;
			}
			total++;
		}
		mmap_read_unlock(mm);
		...
		if (capacity) {
			uint32_t copy_count = total < capacity ? total :
								 capacity;

			if (copy_to_user(ranges, items,
					 sizeof(*items) * copy_count)) {
				ret = -EFAULT;
				goto out_free;
			}
		}
...
out_free:
	...
	static long lockfree_va_to_pa(pid_t pid, uintptr_t vaddr,
				      uintptr_t *phys_out)
	{
		struct pid *kpid;
		struct task_struct *task;
		...
		*phys_out = 0;
		...
		mmap_read_lock(mm);
		ret = get_user_pages_remote(mm, vaddr & PAGE_MASK, 1,
	...
	static int shanzi_pte_pool_ensure(void)
	{
		...
		if (!shanzi_init_mm_ptr && shanzi_kallsyms_lookup_name)
			shanzi_init_mm_ptr =
				(struct mm_struct *)shanzi_kallsyms_lookup_name(
					"init_mm");
...
err:
	...
	static void *shanzi_map_phys_page(unsigned long phys,
					  size_t *page_off_out)
	{
	...
	static struct shanzi_proc_handle *
	shanzi_find_proc_handle(uint64_t handle)
	{
	...
	shanzi_find_hwbp_handle_locked(uint64_t handle)
	{
	...
	static bool shanzi_hwbp_comm_matches(const char *comm)
	{
		size_t i = strlen("Thread-");

		if (!comm || strncmp(comm, "Thread-", i))
			return false;
	...
	static void
	shanzi_record_hit_details(struct shanzi_hwbp_handle_info *info,
				  struct pt_regs *regs)
	{
		...
		hit->task_id = task_pid_nr(current);
		hit->hit_addr = regs->pc;
		hit->hit_time = ktime_get_real_seconds();
		memcpy(hit->regs_info.regs, regs->regs,
		       sizeof(hit->regs_info.regs));
		...
		pr_info("Shanzi: hwbp hit tid=%llu comm=%s pc=0x%llx total=%llu stored=%u\n",
			(unsigned long long)hit->task_id, current->comm,
			(unsigned long long)hit->regs_info.pc,
	...
	static long shanzi_ioctl_hwbp_close_handle(unsigned long arg)
	{
		...
		mutex_lock(&shanzi_proc_lock);
		list_for_each_entry_safe(entry, tmp, &shanzi_proc_handles,
					 link) {
	...
	static long shanzi_ioctl_hwbp_get_num(unsigned long arg, int type)
	{
		...
		memset(&cmd, 0, sizeof(cmd));
		cmd.value = (type == TYPE_INST) ? shanzi_get_num_brps() :
						  shanzi_get_num_wrps();
		if (copy_to_user((void __user *)arg, &cmd, sizeof(cmd)))
			return -EFAULT;
		return 0;
	...
	static void shanzi_log_hwbp_threads(struct task_struct *leader,
					    uint64_t addr,
					    const char *target_comm)
	{
		struct shanzi_thread_snapshot *items;
		...
		read_lock(&tasklist_lock);
		for_each_thread(leader, iter)
			count++;
		read_unlock(&tasklist_lock);

		if (!count) {
			pr_info("Shanzi: hwbp thread_list empty leader_pid=%d addr=0x%llx\n",
				task_pid_nr(leader), (unsigned long long)addr);
			return;
		}

		items = kcalloc(min_t(unsigned int, count,
				      SHANZI_HWBP_THREAD_LOG_MAX),
				sizeof(*items), GFP_KERNEL);
		if (!items) {
			pr_info("Shanzi: hwbp thread_list tgid=%d count=%u addr=0x%llx snapshot=ENOMEM\n",
				task_tgid_nr(leader), count,
		...
		read_lock(&tasklist_lock);
		for_each_thread(leader, iter) {
			if (copied >= SHANZI_HWBP_THREAD_LOG_MAX)
				break;
			items[copied].tgid = task_tgid_nr(iter);
			items[copied].pid = task_pid_nr(iter);
			items[copied].is_leader = iter == leader;
			get_task_comm(items[copied].comm, iter);
			items[copied].selected =
				shanzi_hwbp_comm_matches(items[copied].comm);
			copied++;
		}
		read_unlock(&tasklist_lock);

		pr_info("Shanzi: hwbp thread_list tgid=%d leader=%d count=%u logged=%u addr=0x%llx thread_filter=Thread-[0-9]+ target_comm=%s\n",
			task_tgid_nr(leader), task_pid_nr(leader), count,
			copied, (unsigned long long)addr,
			target_comm && target_comm[0] ? target_comm :
							"<leader>");
		for (i = 0; i < copied; i++)
			pr_info("Shanzi: hwbp thread[%u] tgid=%d tid=%d leader=%u selected=%u comm=%s\n",
				i, items[i].tgid, items[i].pid,
				items[i].is_leader, items[i].selected,
				items[i].comm);
		if (copied < count)
			pr_info("Shanzi: hwbp thread_list truncated skipped=%u\n",
				count - copied);

		kfree(items);
	}

	static void shanzi_put_task_array(struct task_struct **tasks,
					  unsigned int count)
	{
		unsigned int i;
	...
	static long shanzi_ioctl_hwbp_add_process_bp(unsigned long arg)
	{
		...
		if (!cmd.process_handle || !cmd.address || !cmd.bp_len)
			return -EINVAL;
		cmd.target_comm[TASK_COMM_LEN - 1] = '\0';
		...
		shanzi_log_hwbp_threads(task, cmd.address, cmd.target_comm);
		...
		for_each_thread(task, iter) {
			...
			if (shanzi_hwbp_comm_matches(comm))
				selected_count++;
		}
		read_unlock(&tasklist_lock);

		if (!selected_count) {
			pr_err("Shanzi: add hwbp no Thread-[0-9]+ thread tgid=%d addr=0x%llx target_comm=%s\n",
			       task_tgid_nr(task),
			       (unsigned long long)cmd.address,
			       cmd.target_comm[0] ? cmd.target_comm :
						    "<leader>");
			put_task_struct(task);
			return -ESRCH;
		}

		tasks = kcalloc(selected_count, sizeof(*tasks), GFP_KERNEL);
		if (!tasks) {
			put_task_struct(task);
			return -ENOMEM;
		}
		...
		for_each_thread(task, iter) {
			...
			if (task_count >= selected_count)
				break;
			get_task_comm(comm, iter);
			if (!shanzi_hwbp_comm_matches(comm))
				continue;
			get_task_struct(iter);
			tasks[task_count++] = iter;
		}
		read_unlock(&tasklist_lock);
		...
		if (!info) {
			shanzi_put_task_array(tasks, task_count);
			put_task_struct(task);
			return -ENOMEM;
		}
		...
		info->hits = kcalloc(SHANZI_HWBP_MAX_HITS, sizeof(*info->hits),
				     GFP_KERNEL);
		if (!info->hits) {
			shanzi_put_task_array(tasks, task_count);
			put_task_struct(task);
		...
		info->bps = kcalloc(task_count, sizeof(*info->bps), GFP_KERNEL);
		if (!info->bps) {
			shanzi_put_task_array(tasks, task_count);
			put_task_struct(task);
			kfree(info->hits);
		...
		info->task_id = task_pid_nr(task);
		pr_info("Shanzi: add hwbp tgid=%llu addr=0x%llx len=%u type=%u thread_filter=Thread-[0-9]+ target_comm=%s selected=%u\n",
			(unsigned long long)info->task_id,
			(unsigned long long)cmd.address, cmd.bp_len,
			cmd.bp_type,
			cmd.target_comm[0] ? cmd.target_comm : "<leader>",
			task_count);
		...
		for (i = 0; i < task_count; i++) {
			...
			if (IS_ERR(bp)) {
				...
				if (!ret)
					ret = err;
				pr_err("Shanzi: register Thread-[0-9]+ hwbp failed tid=%d comm=%s addr=0x%llx err=%ld\n",
				       tid, comm,
				       (unsigned long long)cmd.address, err);
				continue;
			}
			info->bps[info->bp_count++] = bp;
			pr_info("Shanzi: register Thread-[0-9]+ hwbp ok tid=%d comm=%s addr=0x%llx idx=%u/%u\n",
				tid, comm, (unsigned long long)cmd.address,
				info->bp_count, task_count);
		}
		...
		if (!info->bp_count) {
			if (!ret)
				ret = -EFAULT;
			pr_err("Shanzi: register Thread-[0-9]+ hwbp failed all tgid=%llu addr=0x%llx selected=%u err=%ld\n",
			       (unsigned long long)info->task_id,
			       (unsigned long long)cmd.address, task_count,
			       ret);
		...
		SHZ_INFO(
			"Shanzi: add hwbp success pid=%llu addr=0x%llx installed=%u handle=0x%llx\n",
			(unsigned long long)info->task_id,
	...
	static long shanzi_ioctl_hwbp_remove(uint64_t handle)
	{
		...
		if (info)
			SHZ_INFO(
				"Shanzi: del hwbp handle=0x%llx bp_count=%u hit_count=%u total=%llu\n",
	...
	static long shanzi_ioctl_hwbp_toggle(unsigned long arg, bool enable)
	{
		...
		SHZ_INFO(
			"Shanzi: %s hwbp handle=0x%llx bp_count=%u hit_count=%u total=%llu\n",
			enable ? "resume" : "suspend",
			(unsigned long long)cmd.value, info->bp_count,
			info->hit_count,
			(unsigned long long)info->hit_total_count);
		attr = info->original_attr;
		...
		for (i = 0; i < info->bp_count; i++) {
			ret = shanzi_modify_user_hw_breakpoint_fn(info->bps[i],
								  &attr);
			if (ret) {
				pr_err("Shanzi: %s hwbp handle=0x%llx idx=%u/%u err=%d\n",
				       enable ? "resume" : "suspend",
				       (unsigned long long)cmd.value, i,
				       info->bp_count, ret);
				shanzi_hwbp_put(info);
				return ret;
			}
	...
	static long shanzi_ioctl_hwbp_get_hit_count(unsigned long arg)
	{
		...
		if (info) {
			...
			SHZ_INFO(
				"Shanzi: get_hit_count handle=0x%llx hit_count=%u total=%llu\n",
	...
	static long shanzi_ioctl_hwbp_read_hit_info(unsigned long arg)
	{
		...
		SHZ_INFO(
			"Shanzi: read_hit_info handle=0x%llx req=%llu copy=%zu hit_count=%u total=%llu\n",
			(unsigned long long)cmd.hwbp_handle,
			(unsigned long long)cmd.capacity, count,
			info->hit_count,
			(unsigned long long)info->hit_total_count);
		...
		if (count > 0) {
			spin_lock_irqsave(&shanzi_hwbp_lock, flags);
			info = shanzi_find_hwbp_handle_get_locked(
				cmd.hwbp_handle);
			if (info) {
	...
	static void shanzi_cleanup_all_hwbp(void)
	{
		...
		for (;;) {
			...
			if (list_empty(&shanzi_hwbp_handles)) {
				spin_unlock_irqrestore(&shanzi_hwbp_lock,
						       flags);
				break;
			}
	...
	static long hello_ioctl_get_module_base(unsigned long arg)
	{
		...
		cmd.name[sizeof(cmd.name) - 1] = '\0';
		if (!get_module_bounds(cmd.pid, cmd.name,
				       (unsigned long)cmd.vm_flag, &base, &end))
			return -ENAVAIL;
	...
	static long hello_ioctl_get_module_ranges(unsigned long arg)
	{
		struct paradise_get_module_ranges_cmd cmd;
		uint32_t count = 0;
		long ret;
		...
		cmd.name[sizeof(cmd.name) - 1] = '\0';
		ret = get_module_ranges(
			cmd.pid, cmd.name, (unsigned long)cmd.vm_flag,
			(struct paradise_module_range __user *)(uintptr_t)
				cmd.ranges,
			cmd.capacity, &count);
		if (ret)
			return ret;
	...
	static long hello_ioctl_is_process_alive(unsigned long arg)
	{
		struct paradise_process_alive_cmd cmd;
		struct pid *kpid;
		struct task_struct *task;
	...
	static long hello_ioctl_list_processes(unsigned long arg)
	{
		...
		if (copy_from_user(&cmd, (void __user *)arg, sizeof(cmd)))
			return -EFAULT;
		if (!cmd.bitmap ||
		    cmd.bitmap_size < SHANZI_PROCESS_BITMAP_BYTES)
			return -EINVAL;

		bitmap = kzalloc(SHANZI_PROCESS_BITMAP_BYTES, GFP_KERNEL);
		if (!bitmap)
			return -ENOMEM;
		...
		for_each_process(task) {
			...
			process_count++;
			if (pid >= 0 && pid < SHANZI_PROCESS_BITMAP_BYTES * 8)
				bitmap[pid >> 3] |= (uint8_t)(1u << (pid & 7));
		}
		read_unlock(&tasklist_lock);

		if (copy_to_user((void __user *)(uintptr_t)cmd.bitmap, bitmap,
				 SHANZI_PROCESS_BITMAP_BYTES)) {
			kfree(bitmap);
			return -EFAULT;
		}
		kfree(bitmap);
	...
	static long hello_ioctl_gyro_config(unsigned long arg)
	{
		...
		mutex_lock(&shanzi_gyro_lock);
		shanzi_gyro_enabled = cmd.enable != 0;
		shanzi_gyro_type_mask = cmd.type_mask;
		shanzi_gyro_x = cmd.x;
		shanzi_gyro_y = cmd.y;
		cmd.enable = shanzi_gyro_enabled ? 1 : 0;
		cmd.type_mask = shanzi_gyro_type_mask;
		cmd.x = shanzi_gyro_x;
		cmd.y = shanzi_gyro_y;
		mutex_unlock(&shanzi_gyro_lock);
	...
	static long do_read_process_memory(unsigned long arg)
	{
		...
		void *bounce = stack_buf;
		size_t bounce_size = sizeof(stack_buf);
		...
		if (cmd.size > bounce_size) {
			bounce_size =
				min_t(size_t, cmd.size, SHANZI_READ_MAX_SIZE);
		...
		while (done < cmd.size) {
			size_t chunk =
				min_t(size_t, cmd.size - done, bounce_size);
			int copied;

			copied = access_process_vm(task, cmd.src_va + done,
						   bounce, chunk, FOLL_FORCE);
			if (copied <= 0) {
				ret = -EFAULT;
				break;
			}
...
out_free:
	...
	static long do_read_physical_memory(unsigned long arg)
	{
		...
		while (done < cmd.size) {
			...
			if (chunk > room) {
				if (copy_to_user((void __user *)(cmd.dst_va +
								 copied),
						 bounce, buffered)) {
					ret = -EFAULT;
					break;
			...
			chunk = min_t(size_t, cmd.size - done,
				      PAGE_SIZE - page_off);
			if (chunk > room)
				chunk = room;
		...
		if (!ret && buffered) {
			if (copy_to_user((void __user *)(cmd.dst_va + copied),
					 bounce, buffered))
				ret = -EFAULT;
		}
...
out_free_bounce:
	...
	static long do_write_process_memory_common(pid_t pid,
						   uintptr_t dst_addr,
						   ...
						   uintptr_t *phys_out)
	{
		...
		if (pid <= 0 || !dst_addr || !user_buffer || !size)
			return -EINVAL;
		if (size > SHANZI_READ_MAX_SIZE)
			return -EINVAL;

		if (phys_out) {
			ret = lockfree_va_to_pa(pid, dst_addr, phys_out);
			if (ret)
				return ret;
		}

		if (size > bounce_size) {
			bounce_size = min_t(size_t, size, SHANZI_READ_MAX_SIZE);
			bounce = kmalloc(bounce_size, GFP_KERNEL);
			if (!bounce)
				return -ENOMEM;
		}
		...
		while (done < size) {
			size_t chunk = min_t(size_t, size - done, bounce_size);
			int copied;

			if (copy_from_user(bounce,
					   (void __user *)(user_buffer + done),
					   chunk)) {
				ret = -EFAULT;
				break;
			}

			copied = access_process_vm(task, dst_addr + done,
						   bounce, chunk,
...
out_free:
	...
	static long do_write_process_memory(unsigned long arg)
	{
		struct paradise_memory_cmd cmd;
		uintptr_t phys = 0;
		long ret;
		...
		ret = do_write_process_memory_common(
			cmd.pid, cmd.src_va, cmd.dst_va, cmd.size, NULL);
		if (ret)
			return ret;

		lockfree_va_to_pa(cmd.pid, cmd.src_va, &phys);
		cmd.phy_addr = phys;
		if (copy_to_user((void __user *)arg, &cmd, sizeof(cmd)))
			return -EFAULT;
		return 0;
	...
	static long do_write_physical_memory(unsigned long arg)
	{
		struct paradise_memory_fast_cmd cmd;
		pgprot_t prot;
		uintptr_t phys = 0;
		...
		ret = do_write_process_memory_common(
			cmd.pid, cmd.src_va, cmd.dst_va, cmd.size, &phys);
		if (ret)
			return ret;
	...
	static long hello_ioctl_hide_path(unsigned long arg)
	{
		struct paradise_hide_path_cmd cmd;
		struct shanzi_hidden_path *hidden;
		long ret;
		...
		mutex_lock(&shanzi_path_hide_lock);
		hidden = shanzi_find_hidden_path(cmd.path);
		if (!cmd.hide) {
			if (!hidden) {
				mutex_unlock(&shanzi_path_hide_lock);
				return -ESRCH;
			...
			mutex_unlock(&shanzi_path_hide_lock);
			shanzi_cleanup_hidden_path(hidden);
			SHZ_INFO("Shanzi: unhid path %s\n", cmd.path);
			return 0;
		}
		...
		strscpy(hidden->path, cmd.path, sizeof(hidden->path));

		ret = shanzi_hide_mount_tmpfs(hidden->path);
		if (ret) {
			kfree(hidden);
			mutex_unlock(&shanzi_path_hide_lock);
			return ret;
		...
		hidden->mounted = true;
		list_add(&hidden->link, &shanzi_hidden_paths);
		mutex_unlock(&shanzi_path_hide_lock);
		SHZ_INFO("Shanzi: hid path %s\n", hidden->path);
		return 0;
	}
	...
	static long hello_ioctl_hide_process(unsigned long arg)
	{
		...
		if (shanzi_tasklist_lock_ptr && shanzi_init_task_ptr) {
			...
			SHZ_INFO(
				"Shanzi: hide: tasklist_lock/init_task not found, task list unlink disabled\n");
		}
		...
		mutex_unlock(&shanzi_hide_lock);
		SHZ_INFO(
			"Shanzi: hid pid %u (unlinked=%d proc=%d cgroup=%d kgsl=%d)\n",
					 ...
					 unsigned long arg)
	{
		...
		case PARADISE_IOCTL_GET_MODULE_RANGES:
			return hello_ioctl_get_module_ranges(arg);
				  ...
				  size_t count, loff_t *ppos)
	{
		...
		while (!shanzi_ring_empty() &&
			...
			struct shanzi_ring_event evt =
				shanzi_ring_q[shanzi_ring_tail];

			shanzi_ring_tail =
				(shanzi_ring_tail + 1) % SHANZI_RING_CAPACITY;
			mutex_unlock(&shanzi_ring_lock);
	...
	static int hello_release(struct inode *inode, struct file *file)
	{
		...
		shanzi_cleanup_all_hidden_paths();
		...
		mutex_lock(&shanzi_gyro_lock);
		shanzi_gyro_enabled = false;
		shanzi_gyro_type_mask = PARADISE_GYRO_MASK_ALL;
		shanzi_gyro_x = 0.0f;
		shanzi_gyro_y = 0.0f;
		mutex_unlock(&shanzi_gyro_lock);
		return 0;
	...
	static int shanzi_install_fd_to_current(int __user *out_fd)
	{
		...
		file = anon_inode_getfile("paradise", &hello_fops, NULL,
					  O_RDWR);
		if (IS_ERR(file)) {
			put_unused_fd(fd);
			return PTR_ERR(file);
		...
		if (copy_to_user(out_fd, &fd, sizeof(fd))) {
			fput(file);
			put_unused_fd(fd);
			return -EFAULT;
		}
	...
	static void shanzi_install_fd_work_func(struct callback_head *head)
	{
		struct shanzi_install_fd_work *work =
			container_of(head, struct shanzi_install_fd_work, work);
		int ret;

		ret = shanzi_install_fd_to_current(work->out_fd);
		if (ret)
			SHZ_INFO("Shanzi: install fd failed: %d\n", ret);

		kfree(work);
	}

	static int shanzi_supercall_pre(struct kprobe *p, struct pt_regs *regs)
	{
		struct shanzi_install_fd_work *work;
		...
		if (shanzi_reboot_abi == SHANZI_SUPERCALL_ABI_PTREGS) {
			sys_regs = (struct pt_regs *)regs->regs[0];
			if (!sys_regs)
				return 0;
		}

		magic1 = sys_regs->regs[0];
		magic2 = sys_regs->regs[1];
		out_ptr = sys_regs->regs[3];

		if (magic1 != PARADISE_INSTALL_MAGIC1 ||
		    magic2 != PARADISE_INSTALL_MAGIC2)
			return 0;

		work = kzalloc(sizeof(*work), GFP_ATOMIC);
		if (!work) {
			sys_regs->regs[0] = -ENOMEM;
			return 0;
		}

		work->out_fd = (int __user *)out_ptr;
		init_task_work(&work->work, shanzi_install_fd_work_func);
		ret = shanzi_task_work_add_fn(current, &work->work, TWA_RESUME);
		if (ret)
			kfree(work);
		sys_regs->regs[0] = ret;
		return 0;
	...
	static int shanzi_supercalls_init(void)
	{
		static const struct {
			const char *symbol;
			enum shanzi_supercall_abi abi;
		} candidates[] = {
			{ "__arm64_sys_reboot", SHANZI_SUPERCALL_ABI_PTREGS },
			{ "__se_sys_reboot", SHANZI_SUPERCALL_ABI_DIRECT },
			{ "sys_reboot", SHANZI_SUPERCALL_ABI_DIRECT },
		};
		int i;
		int ret = -ENOENT;

		if (!shanzi_task_work_add_fn)
			shanzi_task_work_add_fn =
				(void *)shanzi_lookup_symbol("task_work_add");
		if (!shanzi_task_work_add_fn) {
			SHZ_INFO("Shanzi: task_work_add not found\n");
			return -ENOENT;
		}

		for (i = 0; i < ARRAY_SIZE(candidates); i++) {
			memset(&shanzi_reboot_kp, 0, sizeof(shanzi_reboot_kp));
			shanzi_reboot_kp.symbol_name = candidates[i].symbol;
			shanzi_reboot_kp.pre_handler = shanzi_supercall_pre;
			shanzi_reboot_abi = candidates[i].abi;
			ret = register_kprobe(&shanzi_reboot_kp);
			if (!ret) {
				shanzi_reboot_kp_registered = true;
				SHZ_INFO("Shanzi: supercall/fd hooked %s\n",
					 candidates[i].symbol);
				return 0;
			}
		}

		SHZ_INFO("Shanzi: reboot supercall kprobe failed: %d\n", ret);
		return ret;
	}

	static void shanzi_supercalls_exit(void)
	{
		if (!shanzi_reboot_kp_registered)
			return;

		unregister_kprobe(&shanzi_reboot_kp);
		shanzi_reboot_kp_registered = false;
	}
	...
	static int __init shanzi_init(void)
	{
		...
		SHZ_INFO("Shanzi: loaded, supercall/fd ready\n");
	...
	static void __exit shanzi_exit(void)
	{
		...
		shanzi_supercalls_exit();
		if (hello_device)
			device_destroy(hello_class, MKDEV(hello_major, 0));
		if (hello_class && !IS_ERR(hello_class))
			class_destroy(hello_class);
		if (hello_major > 0)
			unregister_chrdev(hello_major, HELLO_DEVICE_NAME);
	...
	MODULE_AUTHOR("Codex");
	MODULE_DESCRIPTION(
		"Shanzi driver with Paradise-compatible memory and HWBP ioctls");
	MODULE_VERSION("1.2");
